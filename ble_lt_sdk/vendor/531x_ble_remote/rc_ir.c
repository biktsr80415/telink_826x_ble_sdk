...
/* Internal variables --------------------------------------------------------*/
u16 IRCfgDataBuf[IR_CFG_DATA_BUF_SIZE];
...
void IR_Init(eIR_PinTypeDef IR_Pin)
{
	...
	//Initialize timer1
	reg_tmr1_tick = 0;
	reg_tmr1_capt = 108 * 1000 * 16;//timer interval = 108ms

	pwm_set_clk(CLOCK_SYS_CLOCK_HZ,CLOCK_SYS_CLOCK_HZ);
	pwm_set_cycle_and_duty(PWM0,IR_CARRIER_CYCLE, IR_CARRIER_CYCLE/IR_CARRIER_DUTY );//38K, duty = 1/4
	pwm_set_mode(PWM0,PWM_MODE_IR_DMA_FIFO);
	pwm_polarity_set(PWM0, 1);

	memset((u8*)&IRCfgDataBuf, 0, IR_CFG_DATA_BUF_SIZE*2);//clear buffer
	IrState = IR_STATE_IDLE;
}

/**
 * @Brief: IR transmission.
 * @Param: addr1-> address code
 * @Param: addr2-> address code
 * @Param: cmd  -> command code
 * @Return:None.
 */
void IR_Send(u8 addr1, u8 addr2, u8 cmd)//flash:124ms
{
	u32 irCode = (((~cmd)|0x00000000)<<24) | ((cmd|0x00000000)<<16) | ((addr2|0x00000000)<<8) | (addr1|0x00000000);

	IR_CfgDataBufTypeDef *p = (IR_CfgDataBufTypeDef *)IRCfgDataBuf;

	p->len = 134;

	//IR start bits
	p->data[0] = 0x8000|(IR_START_CARRIER_TIME * IR_CARRIER_FRE/1000000);
	p->data[1] = 0x0000|(IR_START_NO_CARRIER_TIME * IR_CARRIER_FRE/1000000);

	//IR data bits
	u8 i= 0;
	for( i = 0; i<32; i++)
	{
		if(irCode & (0x00000001<<i))//¡°1¡±
		{
			p->data[2*(i+1)] = 0x8000|(IR_1_CARRIER_TIME * IR_CARRIER_FRE/1000000);
			p->data[2*(i+1)+1] = 0x0000|(IR_1_NO_CARRIER_TIME * IR_CARRIER_FRE/1000000);
		}
		else
		{
			p->data[2*(i+1)] = 0x8000|(IR_0_CARRIER_TIME * IR_CARRIER_FRE/1000000);
			p->data[2*(i+1)+1] = 0x0000|(IR_0_NO_CARRIER_TIME * IR_CARRIER_FRE/1000000);
		}
	}
	p->data[2*(i+1)] = 0x8000|(IR_1_CARRIER_TIME * IR_CARRIER_FRE/1000000);//IR stop bits

	pwm_dma_ir_data_config(IRCfgDataBuf);
	pwm_dma_enable();
	pwm_interrupt_enable(FLD_PWM_IRQ_PWM0_IR_DMA_FIFO_EMPTY);
	IrState = IR_STATE_BUSY;

	//Enable timer1 interrupt
	reg_tmr1_tick = 0;
	reg_irq_src  |= FLD_IRQ_SRC_TMR1;//clear irq status
	reg_irq_mask |= FLD_IRQ_TMR1_EN;
	reg_tmr_ctrl |= FLD_TMR1_EN;
}

/**
 * @Brief: IR repeat code transmission.
 * @Param: None.
 * @Return:None.
 */
void IR_RepeatCodeSend(void)
{
	IR_CfgDataBufTypeDef *p = (IR_CfgDataBufTypeDef *)IRCfgDataBuf;
	...
	//IR start code
	p->data[0] = 0x8000 | (IR_REPEAT_START_CARRIER_TIME * IR_CARRIER_FRE/1000000);
	p->data[1] = 0x0000 | (IR_REPEAT_START_NO_CARRIER_TIME * IR_CARRIER_FRE/1000000);
	p->data[2] = 0x8000 | (IR_REPEAT_CARRIER_TIME * IR_CARRIER_FRE/1000000);
	p->data[2] = 0x0000 | (IR_REPEAT_END_TIME * IR_CARRIER_FRE/1000000);
	...
	//Enable timer1 and timer1 interrupt
	reg_tmr1_tick = 0;
	reg_irq_src  |= FLD_IRQ_SRC_TMR1;//clear irq status
	reg_irq_mask |= FLD_IRQ_TMR1_EN;
	reg_tmr_ctrl |= FLD_TMR1_EN;
...
/**
 * @Brief: IR key release processing.
 * @Param: None.
 * @Return:None.
 */
void IR_KeyRelease(void)
{
	//Disable timer1 and timer1 interrupt.
	reg_tmr1_tick = 0;
	reg_irq_src  |= FLD_IRQ_SRC_TMR1;//clear irq status
	reg_irq_mask &= ~FLD_IRQ_TMR1_EN;
	reg_tmr_ctrl &= ~FLD_TMR1_EN;
 ...
 * @Brief:  Get IR state
 * @Param:  None.
 * @Return: IR state IR_STATE_BUSY or IR_STATE_IDLE.
 */
...
{
	/* PWM Interrupt server routine */
	u32 IntFlag = pwm_get_int_status_flag(FLD_PWM_IRQ_PWM0_IR_DMA_FIFO_EMPTY);

	if(IntFlag)
	{
		pwm_clear_interrupt_status(FLD_PWM_IRQ_PWM0_IR_DMA_FIFO_EMPTY);
		//IrState = IR_STATE_IDLE;
	}

	/* Timer1 Interrupt server routine */
	IntFlag = reg_irq_src;

	if(IntFlag & FLD_IRQ_SRC_TMR1)
	{
...
{
	u8 intFlag = pwm_get_int_status_flag(FLD_PWM_IRQ_PWM0_IR_DMA_FIFO_EMPTY);
