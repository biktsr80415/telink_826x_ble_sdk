...
/* Macro define --------------------------------------------------------------*/
#define IR_CFG_DATA_BUF_SIZE      (67 + 2)
...
/*----------------------------------------------------------------------------*/
	...
	#define IR_START_CARRIER_TIME           (9000)//Unit: us
	#define IR_START_NO_CARRIER_TIME        (4500)//Unit: us

	#define IR_1_CARRIER_TIME               (560)   //Unit: us
	#define IR_1_NO_CARRIER_TIME            (1690-5)//Unit: us
	#define IR_0_CARRIER_TIME               (560)   //Unit: us
	#define IR_0_NO_CARRIER_TIME            (560-5) //Unit: us

	#define IR_REPEAT_START_CARRIER_TIME    (9000)//Unit: us
	#define IR_REPEAT_START_NO_CARRIER_TIME (2250)//Unit: us
	#define IR_REPEAT_CARRIER_TIME          (560 )//Unit: us
	#define IR_REPEAT_END_TIME              (500 )//Unit: us
#else
...
/* IR configuration data structure define */
typedef struct
{
	u32 len;
	u16 data[1];
}IR_CfgDataBufTypeDef;

/* external variables --------------------------------------------------------*/
//extern u16 IRCfgDataBuf[];

/* external functions --------------------------------------------------------*/
void IR_Init(eIR_PinTypeDef IR_Pin);
void IR_Send(u8 addr1, u8 addr2, u8 cmd);
