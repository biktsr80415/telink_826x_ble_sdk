
#include "../../proj/tl_common.h"

#include "../../proj/mcu/watchdog_i.h"
#include "../../vendor/common/user_config.h"
#include "../../proj_lib/rf_drv.h"
#include "../../proj_lib/pm.h"
#include "../../proj_lib/ble/ll/ll.h"
#include "../../proj_lib/ble/blt_config.h"


extern my_fifo_t uart_rxfifo;
extern void user_init();
_attribute_ram_code_ void irq_handler(void)
{
	unsigned char irqS = reg_dma_rx_rdy0;
    if(irqS & FLD_DMA_UART_RX)	//rx
    {
    	reg_dma_rx_rdy0 = FLD_DMA_UART_RX;
    	u8* w = uart_rxfifo.p + (uart_rxfifo.wptr & (uart_rxfifo.num-1)) * uart_rxfifo.size;
    	if(w[0]!=0)
    	{
    		my_fifo_next(&uart_rxfifo);
    		u8* p = uart_rxfifo.p + (uart_rxfifo.wptr & (uart_rxfifo.num-1)) * uart_rxfifo.size;
    		reg_dma0_addr = (u16)((u32)p);
    	}
    }

    if(irqS & FLD_DMA_UART_TX)	//tx
    {
    	reg_dma_rx_rdy0 = FLD_DMA_UART_TX;
#if __PROJECT_8266_HOST_DONGLE__
		uart_clr_tx_busy_flag();
#endif
    }
}

int main (void) {
	blc_pm_select_internal_32k_crystal();

	cpu_wakeup_init(CRYSTAL_TYPE);

	set_tick_per_us (CLOCK_SYS_CLOCK_HZ/1000000);

	clock_init();

	gpio_init();

	user_init ();

    irq_enable();

	while (1) {
#if (MODULE_WATCHDOG_ENABLE)
		wd_clear(); //clear watch dog
#endif
		main_loop ();
	}
}
