...
int app_event_callback (u32 h, u8 *p, int n)
{
	...
	if (h &HCI_FLAG_EVENT_BT_STD)		//ble controller hci event
	{
		...
		else if(evtCode == HCI_EVT_LE_META)
		{
			...
			else if(subEvt_code == HCI_SUB_EVT_LE_CREATE_CONNECTION_TIMEOUT)
			{
				//notice that: this create connection timeout event is defined by telink, not a standard ble controller event
				//after controller is set to initiating state by host (blc_ll_createConnection(...) )
				//it will scan the specified device(adr_type & mac), there is a possibility that master controller
				//can not find this specified adv packet(for example the slave is powered off), when this situation happens,
				//master controller will go back to IDLE state after a TIMEOUT(default is 1 minite, use can change this value
				//by blc_ll_setCreateConnectionTimeout(...) )
...
///////////// BLE stack Initialization ////////////////
	...
	blc_hci_le_setEventMask_cmd(		HCI_LE_EVT_MASK_CONNECTION_COMPLETE  \
									...
									|   HCI_LE_EVT_MASK_CREATE_CONNECTION_TIMEOUT);
