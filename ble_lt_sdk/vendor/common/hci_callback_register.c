...
int rx_from_uart_cb (void)//UART data send to Master,we will handler the data as CMD or DATA
{
	...
	if (rx_len)
	{
		cpu_sleep_wakeup(0,0,1);
//		bls_uart_handler(T_rxdata_for_flyco.data, rx_len - 4);//todo:define your own handler for SPP CMD/DATA
	}
...
para[0] = status;
blc_hci_send_data (header, para, para_len);
return 0;
**/
