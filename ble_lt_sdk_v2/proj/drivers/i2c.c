...
void i2c_pin_init(I2C_GPIO_GroupTypeDef i2c_pin_group){
...
#elif (MCU_CORE_TYPE == MCU_CORE_MSSOC)
	...
	}else if(i2c_pin_group == I2C_GPIO_GROUP_D4D5){
		gpio_sda = GPIO_PD5;
		gpio_scl = GPIO_PD4;
	...
	gpio_set_input_en(gpio_sda, 1);
	gpio_set_input_en(gpio_scl, 0);
	gpio_set_output_en(gpio_sda, 1);
	gpio_set_output_en(gpio_scl, 1);
...
void i2c_master_init_div(unsigned char slave_id, unsigned char div_clock)
{
	...
	BIT_CLR(REG_ADDR8(0x73), MASK_VAL(BIT_RNG(4, 7) , 0x01));
	BIT_SET(REG_ADDR8(0x73), MASK_VAL(BIT_RNG(4, 7) , 0x00));///0:4x 1:8x
...
void i2c_write_byte(unsigned char* addr, int addr_len, unsigned char data)
{
	...
	if(addr_len == 1){
		...
		reg_i2c_adr = addr[0] & 0x04;///only low 4bits used
		reg_i2c_do  = addr[1];
		reg_i2c_di	= addr[2];
		reg_i2c_ctrl = MASK_VAL(FLD_I2C_CMD_START, 1, FLD_I2C_CMD_ID, 1, FLD_I2C_CMD_ADR, 1, FLD_I2C_CMD_DO, 1, FLD_I2C_CMD_DI, 1);
	}
	else{
		while(1);
	}
...
unsigned char i2c_read_byte(unsigned char* addr, unsigned char addr_len)
{
	...
	if(addr_len == 1){
		...
		reg_i2c_adr = addr[0] & 0x0f;///only low 4bits used
		reg_i2c_do  = addr[1];
		reg_i2c_di	= addr[2];
		reg_i2c_ctrl = MASK_VAL(FLD_I2C_CMD_START, 1, FLD_I2C_CMD_ID, 1, FLD_I2C_CMD_ADR, 1, FLD_I2C_CMD_DO, 1, FLD_I2C_CMD_DI, 1);
	}
 ...
 */
void i2c_write_dma(unsigned int addr, unsigned char addr_len, unsigned char* pbuf, int len)
{
	int idx = 0;	
	...
	if(addr_len == 1){
		...
		reg_i2c_adr = (unsigned char)((addr>>16) & 0x0f);///only low 4bits used
 ...
 */
void i2c_read_dma(unsigned int addr, unsigned char addr_len, unsigned char* pbuf, int len)
{
	int idx = 0;
	...
	if(addr_len == 1){
		...
		reg_i2c_do  = (unsigned char)(addr>>8);
		reg_i2c_di	= (unsigned char)addr;
		reg_i2c_ctrl = MASK_VAL(FLD_I2C_CMD_START, 1, FLD_I2C_CMD_ID, 1, FLD_I2C_CMD_ADR, 1, FLD_I2C_CMD_DO, 1, FLD_I2C_CMD_DI, 1, FLD_I2C_CMD_STOP, 1);
	}
	else{
		while(1);
	}
