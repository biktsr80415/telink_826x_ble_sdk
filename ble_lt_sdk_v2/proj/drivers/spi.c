...
#elif((MCU_CORE_TYPE == MCU_CORE_8261)||(MCU_CORE_TYPE == MCU_CORE_8267)||(MCU_CORE_TYPE == MCU_CORE_8269)|| (MCU_CORE_TYPE == MCU_CORE_MSSOC))
...
#endif
{
		...
		if(data_clk_pin == SPI_PIN_GROUPA){///group A
			gpio_set_func(GPIO_PA2,AS_SPI);///do
			gpio_set_func(GPIO_PA3,AS_SPI);
			gpio_set_func(GPIO_PA4,AS_SPI);///di

			gpio_set_input_en(GPIO_PA2, 1);
			gpio_set_input_en(GPIO_PA3, 1);
			gpio_set_input_en(GPIO_PA4, 1);

			///disable the other group pin

			///disable the other group pin
			BM_SET(REG_ADDR8(0x5ae),BIT(4) | BIT(5) | BIT(6) | BIT(7));///pd2 pd3
			BM_SET(REG_ADDR8(0x5af),BIT(0) | BIT(1) | BIT(2) | BIT(3));///pd4 pd5
		...
		}else if(data_clk_pin == SPI_PIN_GROUPD){///group D
			gpio_set_func(GPIO_PD3,AS_SPI);///do
			gpio_set_func(GPIO_PD4,AS_SPI);
			gpio_set_func(GPIO_PD5,AS_SPI);///di
			...
			///disable the other group pin
			BM_SET(REG_ADDR8(0x5a8),BIT(4) | BIT(6));///pa2 pa3
			BM_SET(REG_ADDR8(0x5a9),BIT(0));///pa4
			BM_SET(REG_ADDR8(0x5af),BIT(5));///pd6
...
#endif
{
	/***config the corresponding gpio as spi and enable or disable other function***/
	...
	#elif(MCU_CORE_TYPE == MCU_CORE_MSSOC)
		if(spi_grp == SPI_PIN_GROUPA){///group A
			gpio_set_func(GPIO_PA2,AS_SPI);///do
			gpio_set_func(GPIO_PA3,AS_SPI);
			gpio_set_func(GPIO_PA4,AS_SPI);///di
			gpio_set_func(GPIO_PD6,AS_SPI);

			gpio_set_input_en(GPIO_PA2, 1);
			gpio_set_input_en(GPIO_PA3, 1);
			gpio_set_input_en(GPIO_PA4, 1);
			gpio_set_input_en(GPIO_PD6, 1);
		...
		}else if(spi_grp == SPI_PIN_GROUPD){///group D
			gpio_set_func(GPIO_PD2,AS_SPI);
			gpio_set_func(GPIO_PD3,AS_SPI);///do
			gpio_set_func(GPIO_PD4,AS_SPI);
			gpio_set_func(GPIO_PD5,AS_SPI);///di
