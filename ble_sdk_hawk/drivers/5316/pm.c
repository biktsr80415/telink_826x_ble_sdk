...
const TBLCMDSET  tbl_cpu_wakeup_init[] = {
	...
	{0x620, 0x01,  TCMD_UNDER_BOTH | TCMD_WRITE},//Timer0 Enable. for watchdog
 ...
 * @ReVal: None.
 */
void MCU_32kClockSourceSelect(eLSC_32kSrcTypeDef LSC_32kSrc)
{
...
//	todo: avoid cpu stall wakeup by timer0
	...
	flash_read_page(0x8, 1, (unsigned char *)&flashFlag);
	if(flashFlag == 0x4b)
	{
		ota_program_offset = 0x20000;
	}
...
void cpu_set_gpio_wakeup (eGPIO_PinTypeDef pin, int pol, int en) {
	///////////////////////////////////////////////////////////
...
//	analog_write(0x81,0xcf);//Increase XTAL current.(user for PM driver test.)
	...
	if(long_suspend){
		...
		//tick_cur += (unsigned int)((tick_32k<<1) - (tick_32k_cur<<1) - 3) / 32 * tick_32k_calib;
	}
	else{
		tick_cur += (unsigned int)(tick_32k - tick_32k_cur) * tick_32k_calib / 16 ;		// current clock
		//tick_cur += (unsigned int)((tick_32k<<1) - (tick_32k_cur<<1) - 3) * tick_32k_calib / 32;
	}

	REG_ADDR8(0x66) = reg66;//restore system clock

	//if (tick_cur >= wakeup_tick)
	//if((wakeup_src & PM_WAKEUP_TIMER) && (unsigned int)((wakeup_tick - 16) - tick_cur ) > BIT(30))
...
//	if(anareg44 & WAKEUP_STATUS_TIMER)//bit1
//	{
//		DBG_CHN3_TOGGLE;
//	}

//	if(anareg44 & WAKEUP_STATUS_CORE)//bit2
//	{
//		DBG_CHN4_TOGGLE;
//	}
//
//	if(anareg44 & WAKEUP_STATUS_PAD)//bit3
//	{
//		DBG_CHN5_TOGGLE;
//	}
//
//	if((anareg44 & (WAKEUP_STATUS_TIMER|WAKEUP_STATUS_CORE|WAKEUP_STATUS_PAD)) == 0)
//	{
//		DBG_CHN3_TOGGLE;
//	}
