...
typedef enum{
	RF_MODE_ZIGBEE = 0x00,//Currently can't be used
	RF_MODE_BLE_1M = 0x01,
	RF_MODE_BLE_2M = 0x02,//Currently can't be used
}eRF_ModeTypeDef;
...
static inline void pga_enable (int en) {
	unsigned char st = ReadAnalogReg (0x07);		//bit 2
	if (en) {
...
static inline void rf_ble_stx(void* addr, unsigned int tick)
{
