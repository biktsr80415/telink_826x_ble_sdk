...
void i2c_pin_init(eI2C_PinTypeDef i2c_pin){
	...
	switch(i2c_pin)
	{
	case I2C_PIN_GROUP_M_A1A2://Master
		gpio_scl = GPIO_PA1;
		gpio_sda = GPIO_PA2;

		GPIOA_AF->RegBits.P1_AF = GPIOA1_I2C_MCK;
		GPIOA_AF->RegBits.P2_AF = GPIOA2_I2C_MSD;
	...
	case I2C_PIN_GROUP_S_B6B7:
		gpio_scl = GPIO_PB6;
		gpio_sda = GPIO_PB7;

		GPIOB_AF->RegBits.P6_AF = GPIOB6_I2C_CK;
		GPIOB_AF->RegBits.P7_AF = GPIOB7_I2C_SD;
...
void i2c_master_init_div(unsigned char slave_id, unsigned char div_clock)
{
	...
	BM_SET(reg_clk_en0,FLD_CLK0_I2C_EN);       //enable i2c clock
