...
void usb_handle_std_intf_req() {
	...
	unsigned char index_l = (control_request.wIndex) & 0xff;
#endif
	switch (value_h) {
...
void usb_handle_out_class_intf_req(int data_request) {
	...
	unsigned char Entity = (control_request.wIndex >> 8) & 0xff;
#endif
...
void usb_handle_in_class_intf_req() {
	unsigned char property = control_request.bRequest;
#if (USB_MIC_ENABLE || USB_SPEAKER_ENABLE)
	unsigned char value_h = (control_request.wValue >> 8);
	unsigned char Entity = (control_request.wIndex >> 8);
#endif
	switch (property) {
		case 0x00:
...
void usb_handle_in_class_endp_req() {
	unsigned char property = control_request.bRequest;
	unsigned char ep_ctrl = control_request.wValue >> 8;
#if (USB_MIC_ENABLE || USB_SPEAKER_ENABLE)
	//unsigned char addr = (control_request.wIndex >> 8);
...
#if 0
	unsigned char property = control_request.bRequest;
	unsigned char ep_ctrl = control_request.wValue & 0xff;
#if (USB_MIC_ENABLE || USB_SPEAKER_ENABLE)
	unsigned char addr = (control_request.wIndex >> 8);
#endif
#endif
...
#if (USB_SPEAKER_ENABLE || USB_MIC_ENABLE)
	unsigned char value_l = (control_request.wValue) & 0xff;
	unsigned char intf_index = (control_request.wIndex) & 0x07;
