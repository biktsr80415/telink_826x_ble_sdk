...
#define MY_ADV_INTERVAL_MAX			ADV_INTERVAL_30MS//ADV_INTERVAL_35MS
...
#endif//end of BLE_REMOTE_OTA_ENABLE
...
void ui_enable_mic (u8 en)
{
	...
	if(en){  //Audio ON
		...
		reg_dfifo_mode &= ~DFIFO_Mode_FIFO0_Input;
	}
...
void blc_checkConnParamUpdate(void)
{
	if(interval_update_tick && clock_time_exceed(interval_update_tick,5*1000*1000)
	   ...
	   && bls_ll_getConnectionInterval()!= 8 )
	{
...
void user_init()
{
	...
	if(analog_read(DEEP_ANA_REG2) == BATTERY_VOL_LOW){
		TL_BattteryCheckProc(BATTERY_VOL_MIN + 200);//2.2V
	}
#endif
	...
	blt_soft_timer_add(&gpio_test3, 17000);
#endif
...
void main_loop (void)
{
	...
	#if(BATT_CHECK_ENABLE)
		if(battery_get_detect_enable() && clock_time_exceed(batteryCheckStartTick, 100*1000)){
			batteryCheckStartTick = clock_time();
			TL_BattteryCheckProc(BATTERY_VOL_MIN);//2.0V
		}
	#endif
