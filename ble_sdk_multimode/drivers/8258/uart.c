...
{
    if (Enable) {
    	BM_CLR(reg_gpio_gpio_func(GPIO_PC4), 0);//disable GPIOC_GP4 Pin's GPIO function
//    	reg_uart_gpio_duplex |= BIT(4); //enable GPIOC_GP4 Pin as RTS Pin
        reg_uart_ctrl2 |= FLD_UART_CTRL2_RTS_EN; //enable RTS function
    }
    else {
    	BM_SET(reg_gpio_gpio_func(GPIO_PC4), 1);//enable GPIOC_GP4 Pin's GPIO function
...
{
    if (Enable) {
    	BM_CLR(reg_gpio_gpio_func(GPIO_PC5), 0);//disable GPIOC_GP5 Pin's GPIO function
//    	reg_uart_gpio_duplex |= BIT(5); //enable GPIOC_GP5 Pin as CTS Pin
    	reg_uart_ctrl1|= FLD_UART_CTRL1_CTS_EN; //enable CTS function
    }
    else {
    	BM_SET(reg_gpio_gpio_func(GPIO_PC5), 1);//enable GPIOC_GP5 Pin's GPIO function
