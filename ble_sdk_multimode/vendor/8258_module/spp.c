...
int event_handler(u32 h, u8 *para, int n)
{
	...
	if((h&HCI_FLAG_EVENT_TLK_MODULE)!= 0)			//module event
	{
		...
		switch(event)
		{
			...
			case BLT_EV_FLAG_SMP_PINCODE_PROCESS:
			{
			...
			case BLT_EV_FLAG_PAIRING_END:
				#if (SECURE_CONNECTION_ENABLE && SMP_NUMERIC_COMPARISON && SMP_UART_ENABLE)
					pm_ctrl_flg = 0;
				#endif
				pairing_end_status = *para;
				if(!pairing_end_status){
//					printf("Pair success\n");
				}
			break;
...
int bls_uart_handler (u8 *p, int n)
{
	...
	else if (spp_cmd == SPP_CMD_GET_CONN_PARA)
	{
		...
		if(interval){
			...
			pEvt->param[0] = HCI_ERR_CONN_NOT_ESTABLISH;
		}
