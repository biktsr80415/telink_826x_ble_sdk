...
function bls_ll_setScanRspData(status, scanrspDataLen, ...)
arg = {...} 

hci_type_cmd = HCI_TYPE_CMD
opcode_OCF = HCI_CMD_LE_SET_SCAN_RSP_DATA
opcode_OGF = HCI_CMD_LE_OPCODE_OGF
cmd_param_len = scanrspDataLen + 1
...
-------for i=6,5+scanrspDataLen,1 do print(cmd[i]) end  
end
...
print(string.format("\t\t\t\tHCI_LE_Set_Advertising_Data") )
print("<-------------------------------------------------------------------------------------")
print(string.format("\t\t\t\t\t(Data Length:0x%02x)", scanrspDataLen) )
