...
void CTl_ble_moduleDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_BTSTACK, m_btStack);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTl_ble_moduleDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTSTACK, OnBtstack)
	//}}AFX_MSG_MAP

	ON_MESSAGE(WM_APPENDLOG, OnAppendLog)
	ON_MESSAGE(WM_EXECCMD, OnExecCmd)
...
extern int usb_receive_packet (unsigned __int8 *p, int n);

int		UsbSendMessage (unsigned __int8 *p, int n)
{
	::SendMessage(AfxGetMainWnd()->GetSafeHwnd(), WM_EXECCMD, n, (LPARAM)p);
	return 0;
}
    ...
    while ( 1 )    { 
		if (m_hDev != NULL) {
			...
			if (bok) {	// send to log window
				...
				usb_receive_packet ((unsigned __int8 *)buff, nB);
...
LRESULT  CTl_ble_moduleDlg::OnExecCmd (WPARAM wParam, LPARAM lParam )
{
	...
	Bin2Text (buff, pu, n);
	ExecCmd ((LPCTSTR) buff);

	return OnAppendLog (wParam, lParam );
}
...
void CTl_ble_moduleDlg::OnScan() 
{
	...
	
	{
		...
		if (dlg.DoModal () == IDOK)
		{
			WriteFile(m_hDevW, dlg.m_mac, 7, &nB, NULL);
			WaitForSingleObject (NULLEVENT2, 10);
...
void CTl_ble_moduleDlg::OnBtstack() 
{
	// TODO: Add your control notification handler code here
	UpdateData ();
	if (m_btStack)
	{
		extern int main_cpp(int argc, const char * argv[]);
		main_cpp (0, 0);
