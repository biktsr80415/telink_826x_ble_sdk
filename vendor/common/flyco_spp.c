...
u8 adv_enable_flag = 1;//Default: enable broadcast flag
	...
	switch(pp->cmdID) {
		...
		case FLYCO_SPP_CMD_MODULE_GET_RSSI:{
			...
			if(pp->len == 0){
				extern u8 blt_state;
				u8 rssi;
				if(blt_state != BLT_LINK_STATE_CONN)
		...
		case FLYCO_SPP_CMD_MODULE_GET_ADV_ENABLE_FLAG:{
			...
			if(pp->len == 0){
				flyco_spp_module_rsp2cmd(pp->cmdID, &adv_enable_flag, 1);
		    }
		}
		break;
		...
		case FLYCO_SPP_CMD_MODULE_SET_IDENTIFIED:{
			...
			if(pp->len == 6){
				...
				nv_write(NV_FLYCO_ITEM_IDENTIFIED, p->data, 6); //store baud rate param
...
void flyco_spp_onModuleReceivedMasterCmd(flyco_spp_cmd_t *pp) {
	...
	switch(pp->cmdID) {
		...
		case FLYCO_SPP_CMD_MODULE_GET_ADV_ENABLE_FLAG:{
			...
			if(pp->len == 0){
				flyco_spp_received_master_rsp2cmd(pp->cmdID, &adv_enable_flag, 1);
		    }
		}
		break;
